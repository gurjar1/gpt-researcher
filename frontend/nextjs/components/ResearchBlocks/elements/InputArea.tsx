            ...
            input.onchange = async (e) => {
              ...
              if (file) {
                try {
                  ...
                  const truncated = text.slice(0, 3000) + (text.length > 3000 ? '...(truncated)' : '');
                  setPromptValue(`[File: ${file.name}]\n\n${truncated}\n\nPlease analyze this content.`);
                } catch {
                  alert('Could not read file. Please try a TXT or MD file.');
                }
              }
            };
            input.click();
          }}
          className="relative flex h-[40px] w-[40px] shrink-0 items-center justify-center rounded-md text-gray-400 hover:text-teal-400 hover:bg-gray-800/50 transition-all duration-300 z-10 mr-2"
