...
import { FocusMode } from "@/components/FocusModeSelector";
import { useQuickSearch } from "@/hooks/useQuickSearch";
import QuickAnswer from "@/components/QuickAnswer";
import QuickSearchHome from "@/components/QuickSearchHome";
...
import AppLayout from "@/utils/getLayout";
...
export default function Home() {
  ...
  // Quick search hook
  const { isLoading: quickSearchLoading, currentAnswer: quickCurrentAnswer, sources: quickSources, query: quickQuery, error: quickError, search: quickSearch, reset: resetQuickSearch, messages: quickMessages, restoreMessages: restoreQuickMessages } = useQuickSearch();
  ...
  return (
    <>
      ...
      ) : !showResult && !showQuickAnswer ? (
        // Desktop view - Unified homepage for all modes (sidebar + animations)
        <QuickSearchHome
          promptValue={promptValue}
          setPromptValue={setPromptValue}
          focusMode={focusMode}
          ...
          onSubmit={(query: string) => {
            if (focusMode === 'deep') {
              // Deep mode triggers the research flow
              handleDisplayResult(query);
            } else {
              // Quick/Reddit/News/Shopping use quick search
              setShowQuickAnswer(true);
              quickSearch(query, focusMode);
            }
          }}
          onRestoreChat={(messages) => {
            setShowQuickAnswer(true);
            restoreQuickMessages(messages);
          }}
        />
      ) : !showResult && showQuickAnswer ? (
        // Desktop view - Quick Answer Chat (no header)
        <QuickAnswer
          messages={quickMessages}
          currentAnswer={quickCurrentAnswer}
          currentSources={quickSources}
