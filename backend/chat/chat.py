...
class ChatAgentWithMemory:
    def __init__(
        ...
        # Use SearXNG for local search (instead of Tavily)
        self.searx_url = os.environ.get("SEARX_URL", "http://localhost:8888")
    ...
    def quick_search(self, query):
        """Perform a web search for current information using SearXNG (local)"""
        try:
            logger.info(f"Performing web search for: {query}")
            
            # Use SearXNG for local search
            with httpx.Client() as client:
                ...
                if response.status_code == 200:
                    ...
                    # Format results similar to Tavily format
                    results = {
                        "results": [
                            {
                                "title": r.get("title", ""),
